...
function ED_evolve_block_diag_distributed(NN, M, XXZCoeff, Jmat, init_state)
...
%     tic;
    ...
    %% evolving the state (distributed)
    [C,D] = size(evolver_blk);
    finalC = round(C - 3*round(0.25*C));
    finalElen = round(length(eigvecs_blk) - ...
        round(0.15*length(eigvecs_blk)) - ...
        round(0.20*length(eigvecs_blk)) - ...
        round(0.32*length(eigvecs_blk)));

    spmd   
        psiEig_i_times_dist = codistributed(psiEig_i_times_blk,...
            codistributor1d(1,[round(0.25*C),...
            round(0.25*C),...
            round(0.25*C),...
            finalC]));

        evolver_dist = codistributed(evolver_blk,...
            codistributor1d(1,[round(0.25*C),...
            round(0.25*C),...
            round(0.25*C),...
            finalC]));

        eigvecs_dist = codistributed(eigvecs_blk,...
            codistributor1d(1,[round(0.15*length(eigvecs_blk)),...
            round(0.20*length(eigvecs_blk)),...
            round(0.32*length(eigvecs_blk)),...
            finalElen]));

        psiEig_f_times_blk = psiEig_i_times_dist .* evolver_dist;

        %convert the state back to original basis and normalize
        psi_f_times_blk = eigvecs_dist * psiEig_f_times_blk;
