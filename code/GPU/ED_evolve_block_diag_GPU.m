...
function [t_make_ham, t_all_blks, block_timing_eig, block_timing_evol, t_meas] = ED_evolve_block_diag_GPU(NN, M, XXZCoeff, Jmat, init_state)
...
make_ham = tic;
%Ham_big = gpuArray(sparse(2^NN, 2^NN));

%Ham_big = gpuArray(zeros(2^NN, 2^NN));
...
block_timing_eig = zeros(1, NN+1);
block_timing_evol = zeros(1, NN+1);
all_blks = tic;
for blk_num = 1:(NN+1)
    ...
    Ham_blk = gpuArray(Ham_big(selected_indx, selected_indx));
    %psi_blk = gpuArray(psi_i(selected_indx));
...
t_all_blks = toc(all_blks)

%% measure Z polarization in the most bruteforce way
meas = tic;
SN = gpuArray(sparse(2^NN, 2^NN));
%SN = zeros(2^NN, 2^NN);
...
Filename = sprintf('N_%d', NN);
writematrix(polarization,Filename);
