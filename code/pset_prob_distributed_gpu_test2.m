...
%change of basis
[eigvecs, eigvals] = eig(full(ham));
psiEig_i = inv(eigvecs)*psi_i; %coordinate transformation
psiEig_i_times = repmat(psiEig_i, 1, M);
eigs_ts = diag(eigvals) * t;
evolver = exp(-1i*eigs_ts);
%--------------------------------------------------------------------------
%% Testing on Cluster 

%evolving the state (distribtued + gpu)
[C,D] = size(evolver);
parpool(parcluster('local'), gpuDeviceCount());

%Time them boiz
tic;
spmd    
    %%eigvecs has the largest dimensions (2^N x 2^N), so split the matrix
    %%unevenly amongst workers for load balancing. psiEig_i_times and evolver
    %%will be (2^N x 1000) which is smaller than eigvecs for large N, split
    %%evenly amongst workers.
    psiEig_i_times_dist = codistributed(psiEig_i_times,...
        codistributor1d(1,[round(0.25*C),...
        round(0.25*C),...
        round(0.25*C),...
        round(0.25*C)]));
    
    evolver_dist = codistributed(evolver,...
        codistributor1d(1,[round(0.25*C),...
        round(0.25*C),...
        round(0.25*C),...
        round(0.25*C)]));
    
    eigvecs_dist = codistributed(eigvecs,...
        codistributor1d(1,[round(0.15*length(eigvecs)),...
        round(0.20*length(eigvecs)),...
        round(0.32*length(eigvecs)),...
        round(0.33*length(eigvecs))]));
    
    psiEig_f_times = psiEig_i_times_dist .* evolver_dist;
    psi_f_times = eigvecs_dist * psiEig_f_times;  
...
%density matrix
%density matrices for different times
% Rho_tot = zeros(size(psi_f_times, 1));

%convert to GPU arrays
% Rho_tot = gpuArray(Rho_tot);
% 
% for i = 1:M
