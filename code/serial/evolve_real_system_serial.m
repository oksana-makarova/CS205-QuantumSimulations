...
%version for serial version testing with CPUs
% Oksana, 05/04/2021
...
if strcmp(P.Simulator,'newED')
    for i = 7:P.N
        ...
        [t_make_Ham, t_all_blks, block_timing_eig, block_timing_evol, t_meas] = ED_evolve_block_diag_serial(i, P.nTimePoints, P.XXZCoeff, JMat, P.InitStates);
        t_tot = toc(tot);
...
7:P.N %number of spins
time_Ham %time it took to construct the Hamiltonian
time_all_blks %time for evolution of a single block
time_block_timing_eig %time it took to diagonalize each block
time_block_timing_evol %time it took to evolve the wavefunction
time_meas %time it took for measurement matrix
time_tot %total time
