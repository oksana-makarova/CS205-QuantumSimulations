...
(For use in evolve_real_system.m and gpuPar.sbatch as helper function)
Parallelization_methods_testing.m - exploration of different parallelization methods that are available from MATLAB’s Parallelization Toolbox: parfor, parfeval, spmd, distributed arrays, co-distributed arrays. The code times (with averaging) each method, with comparison to serial implementation for the following situations: 
...
Random symmetric matrix eigen solving
	Can run as is for testing different timings for large matrices
